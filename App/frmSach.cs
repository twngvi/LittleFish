...
namespace App
{
    public partial class frmSach : Form
    {
        ...
        private bool isExiting = false; // ✅ Biến để tránh gọi xác nhận nhiều lần

        public frmSach()
        {
            InitializeComponent();
            this.txtTuKhoa.KeyDown += new System.Windows.Forms.KeyEventHandler(this.txtTuKhoa_KeyDown);
            this.FormClosing += frmSach_FormClosing; // ✅ Gắn sự kiện xác nhận thoát
        }
        ...
        private void frmSach_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!isExiting && e.CloseReason == CloseReason.UserClosing)
            {
                ...
                {
                    isExiting = true;
                    Application.ExitThread(); // ✅ Thoát ứng dụng ngay lập tức
                }
                else
                {
                    e.Cancel = true;
